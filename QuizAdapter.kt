...
class QuizAdapter(private val quizList: List<Quiz>) :
	...
	val answers = quiz.answers
        holder.textTopLeft.text = answers[0].name
        holder.textTopRight.text = answers[1].name
        holder.textBottomLeft.text = answers[2].name
        holder.textBottomRight.text = answers[3].name

        var date = quiz.publish_at.substring(0, 10)
	date = date + " o godzinie "
	date = date + quiz.publish_at.substring(11, 16)
